...
The `public/` directory contains assets that do not need to be processed by Astro or JavaScript. These files are directly accessible via their URL paths. This folder houses files such as the websites Favicon, Fonts, and Instructions for engine crawlers.

If a file does not require transformations or processing, it most likely goes in `public/`.

#### `src/`

The `src/` directory contains the main logic of the website, including components, pages, API routes, and styles. This is a superfolder that houses the vast majority of the project that contributers will tinker with.
...
If a file is meant to wrap around multiple pages, it's going to go in `layouts/`.

#### `pages/`

The `pages/` directory contains every .astro file that represents an actual webpage that can be visited. It also contains backend functions that fetch and serve data to the frontend within the nested `api/` folder. The `styles/` folder is also present within pages, housing the tradtional CSS classes used throughout the website in the `globals.css` file.
...
```
Aspine/
│── .astro/             # Astro framework settings
...
│   ├── pages/          # Main pages & API endpoints
│   │   ├── api/        # API routes
│   │   ├── dash.astro  # Dashboard page
│   │   ├── index.astro # Homepage
│   │   ├── login.astro # Login page
│   │   ├── license.md  # License information
│   │   ├── hdiw.astro  # "How Does It Work?" page
│   ├── styles/         # Styling
│   │   ├── src/        # Source CSS files
│   │   └── dist/       # Compiled CSS
│   ├── env.d.ts        # Environment variables
│   └── layouts.js      # Layout utilities
│── 
│── 
│── 
│── 
